...
PACKAGE_BASENAME="amphi-ai22-components"
CHECK_ONLY=false
USER_INSTALL=false
PYTHON_OVERRIDE=""
VENV_DIR=""
...
if "jupyterlab" in installed:
    ...
    if current < (4, 3, 4) or current >= (5, 0, 0):
        errors.append("jupyterlab 版本不兼容：需要 >=4.3.4,<5，当前为 %s" % installed["jupyterlab"])
if "jupyterlab-amphi" in installed:
    current = numeric_version(installed["jupyterlab-amphi"])
    if current < (0, 9, 0) or current >= (1, 0, 0):
        errors.append("jupyterlab-amphi 版本不兼容：需要 >=0.9.0,<1，当前为 %s" % installed["jupyterlab-amphi"])
if errors:
