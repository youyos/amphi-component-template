...
PACKAGE_BASENAME="amphi-ai27-components"
LEGACY_PACKAGE_SCOPE="@local-ai27"
LEGACY_PACKAGE_BASENAME="amphi-platform-extension-components"
CHECK_ONLY=false
USER_INSTALL=false
PYTHON_OVERRIDE=""
VENV_DIR=""
...
if [[ "$USER_INSTALL" == true ]]; then
  ...
  LEGACY_TARGET="$JUPYTER_DATA_DIR/labextensions/$LEGACY_PACKAGE_SCOPE/$LEGACY_PACKAGE_BASENAME"
  ...
  LEGACY_TARGET="$DATA_DIR/share/jupyter/labextensions/$LEGACY_PACKAGE_SCOPE/$LEGACY_PACKAGE_BASENAME"
...
if [[ -d "$LEGACY_TARGET" ]]; then
  ...
  LEGACY_BACKUP="$LEGACY_BACKUP_ROOT/${LEGACY_PACKAGE_BASENAME}.scope-migration-${TIMESTAMP}"
  mv "$LEGACY_TARGET" "$LEGACY_BACKUP"
  echo "旧 @local-ai27 扩展已迁移到备份：$LEGACY_BACKUP"
fi
